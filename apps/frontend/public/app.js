...
const NODE_TITLE_BASELINE_OFFSET = 14;
const MAX_NODE_LINE_CHARACTERS = 26;
...
function wrapTextIntoLines(text, maxCharacters = MAX_NODE_LINE_CHARACTERS) {
  ...
  if (explicitLines.length > 1) {
    return explicitLines
      .map((line) => line.trim())
      .flatMap((line) => wrapTextIntoLines(line, maxCharacters));
  }
...
function getMindmapTextSegments(story) {
  if (!story) {
    return [];
  }
  ...
  const pushSegment = (text, classes) => {
    wrapTextIntoLines(text).forEach((line) => {
      segments.push({ text: line, classes });
    });
  };

  pushSegment(story.title || 'Untitled Story', ['story-title']);

  const storyPoint =
    story.storyPoint != null ? `Story Point: ${story.storyPoint}` : 'Story Point: Unestimated';
  pushSegment(storyPoint, ['story-meta']);

  const statusClasses = ['story-meta', 'story-status'];
  const statusClass = getStatusClass(story.status);
  if (statusClass) {
    statusClasses.push(statusClass);
  }
  pushSegment(`Status: ${story.status || 'Draft'}`, statusClasses);

  pushSegment(`Assignee: ${story.assigneeEmail || 'Unassigned'}`, ['story-meta']);

  if (Array.isArray(story.components)) {
    const formatted = story.components
      .map((component) => formatComponentLabel(component) || component)
      .filter((value, index, array) => value && array.indexOf(value) === index)
      .join(', ');
    if (formatted) {
      pushSegment(`Components: ${formatted}`, ['story-meta']);
    }
  }
...
function computeNodeHeightValue(story) {
  const segments = getMindmapTextSegments(story);
  const lineCount = Math.max(segments.length, 1);
  return NODE_VERTICAL_PADDING * 2 + lineCount * NODE_LINE_HEIGHT;
}
...
function renderMindmap() {
  ...
  nodes.forEach((node) => {
    ...
    const baseTextX = node.x + 12;
    const baseLineY = node.y + NODE_VERTICAL_PADDING + NODE_TITLE_BASELINE_OFFSET;
    let lineIndex = 0;

    const appendLine = (text, classNames) => {
      ...
      textEl.setAttribute('x', String(baseTextX));
      textEl.setAttribute('y', String(baseLineY + NODE_LINE_HEIGHT * lineIndex));
    ...
    const textSegments = getMindmapTextSegments(node.story);
    textSegments.forEach((segment) => appendLine(segment.text, segment.classes));
