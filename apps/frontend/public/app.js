    ...
    actions: [
      {
        ...
        onClick: async () => {
          ...
          try {
            ...
            // Auto-open acceptance test modal with autogenerated test
            setTimeout(() => {
              openAcceptanceTestModal(created.id, { autoGenerate: true });
            }, 300);
          } catch (error) {
...
function openAcceptanceTestModal(storyId, options = {}) {
  const { test = null, autoGenerate = false } = options;
  const container = document.createElement('div');
  ...
  // Auto-generate draft when requested (e.g., after child story creation)
  if (autoGenerate && !test) {
    setTimeout(() => {
      loadDraft();
    }, 100);
  }
