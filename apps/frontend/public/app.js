...
function renderCodeWhispererSectionList(container, story) {
  ...
  entries.forEach((entry) => {
    ...
    if (entry.prUrl || (entry.type === 'pull_request' && entry.html_url)) {
      ...
      // Show "PR pending" status with click handler
      const prPending = document.createElement('p');
      prPending.className = 'codewhisperer-pr-link pending';
      prPending.innerHTML = `<span>Pull Request:</span> <a href="#" class="pr-pending-link">⏳ PR being generated...</a>`;
      prPending.querySelector('.pr-pending-link').addEventListener('click', (e) => {
        e.preventDefault();
        showToast('PR is being generated by Kiro. Please wait...', 'info');
      });
      card.appendChild(prPending);
    }
