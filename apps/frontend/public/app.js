...
function openKiroTerminalPage(prEntry) {
  const params = new URLSearchParams();
  if (prEntry?.number || prEntry?.targetNumber) {
    params.set('prId', prEntry.number || prEntry.targetNumber);
  }
  if (prEntry?.taskTitle) {
    params.set('title', prEntry.taskTitle);
  }
  if (prEntry?.prUrl) {
    params.set('prUrl', prEntry.prUrl);
  }
  if (prEntry?.branchName) {
    params.set('branch', prEntry.branchName);
  }

  const terminalUrl = `${window.location.origin}/kiro-terminal.html?${params.toString()}`;
