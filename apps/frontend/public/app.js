...
/**
 * Show modal with all story titles
 */
  ...
  try {
    ...
    if (allStories.length === 0) {
      ...
      displayStories.forEach(story => {
        ...
        item.textContent = `${story.title || 'Untitled'} (ID: ${story.id})`;
      ...
      if (allStories.length > 50) {
        ...
        note.textContent = `Showing 50 of ${allStories.length} stories`;
        container.appendChild(note);
      }
    }
    ...
    console.error('Failed to show all stories:', error);
