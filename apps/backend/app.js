...
module.exports = generatedFunction;`;
    ...
    if (result.success || result.generated) {
      // Check if Kiro CLI already committed the code
      if (result.committed) {
        sendJson(res, 200, { 
          success: true, 
          code: result.code || 'Code committed directly by Kiro CLI',
          source: result.source || 'kiro-direct',
          message: result.message || 'Code generated and committed by Kiro CLI',
          committed: true
        });
        return;
      }
      
      // Fallback: if Kiro CLI provided code but didn't commit, we commit it
      if (result.code && prNumber && branchName) {
        try {
          await commitGeneratedCodeToPR(result.code, prNumber, branchName, prompt);
          console.log(`✅ Generated code committed to PR #${prNumber}`);
          sendJson(res, 200, { 
            success: true, 
            code: result.code,
            source: result.source || 'kiro-fallback-commit',
            message: 'Code generated by Kiro CLI and committed by Lambda',
            committed: true
          });
        } catch (commitError) {
          ...
          sendJson(res, 200, { 
            success: true, 
            code: result.code,
            source: result.source || 'kiro-no-commit',
            message: 'Code generated but commit failed',
            committed: false
          });
        }
      } else {
        sendJson(res, 200, { 
          success: true, 
          code: result.code || result.output || 'Code generated',
          source: result.source || 'kiro-basic',
          message: result.message || 'Code generated by Kiro CLI',
          committed: false
        });
      }
    } else {
