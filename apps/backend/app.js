...
async function getCodebaseContext() {
  try {
    ...
    return `AIPM Project Structure:
${projectStructure}

Key Implementation Files:
${keyFiles}

Architecture: Vanilla JavaScript with Express backend, SQLite/DynamoDB data layer, frontend mindmap interface.
Patterns: RESTful API, modular functions, JSDoc documentation, error handling with try/catch.`;
    
  } catch (error) {
    console.warn('Could not get codebase context:', error.message);
    return 'AIPM: Vanilla JavaScript project with Express backend and frontend mindmap interface.';
  }
}
...
async function getProjectStructure() {
  ...
  return `apps/
  backend/
    app.js - Main backend server with REST API
    server.js - Server startup
  frontend/
    public/
      app.js - Frontend mindmap and UI logic
      index.html - Main HTML page
      styles.css - Styling
scripts/
  kiro-api-server-v3.js - Kiro API server
  contracts/ - JSON transformation contracts`;
}

async function getKeyFiles() {
  // Return key implementation patterns
  return `Backend patterns:
- Express route handlers with sendJson() responses
- DynamoDB operations with safeSelectAll()
- Error handling with try/catch blocks
- JSDoc function documentation

Frontend patterns:
- Vanilla JavaScript DOM manipulation
- Modal dialogs with openModal()
- Toast notifications with showToast()
- API calls with fetch() and resolveApiUrl()`;
}
    ...
    if (storyIdMatch && method === 'PATCH') {
      ...
      try {
        ...
        if (process.env.NODE_ENV !== 'test' && process.env.STORIES_TABLE && process.env.AWS_REGION) {
          ...
          try {
            const updateExpr = ['#status = :status', 'updated_at = :updatedAt'];
            const attrValues = {
              ':status': { S: nextStatus },
              ':updatedAt': { S: now() }
            };
