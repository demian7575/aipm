...
async function performDelegation(payload) {
  ...
  if (normalized.target === 'pr') {
    ...
    try {
      ...
      // Run Amazon Q script directly in CloudShell
      const { execSync } = await import('child_process');
      const scriptPath = '/home/cloudshell-user/aipm/generate-code-with-q.sh';
      
      try {
        execSync(`${scriptPath} "${normalized.taskTitle}" "${taskDetails}" "${branchName}"`, {
          cwd: '/home/cloudshell-user/aipm',
          timeout: 120000,
          stdio: 'inherit'
        });
        
        return {
          type: 'amazon_q_generation',
          message: 'Code generated by Amazon Q with full repository context',
          taskTitle: normalized.taskTitle,
          branchName: branchName,
          prUrl: `https://github.com/${normalized.owner}/${normalized.repo}/compare/${branchName}?expand=1`,
          confirmationCode: `AQ${timestamp}`,
        };
      } catch (execError) {
        console.error('Amazon Q failed, creating empty PR:', execError);
      }
      ...
      try {
        generatedCode = await generateCodeWithBedrock(
      ...
      if (generatedCode && generatedCode.files) {
        for (const file of generatedCode.files) {
          await githubRequest(`${repoPath}/contents/${file.path}`, {
            method: 'PUT',
            body: JSON.stringify({
...
${generatedCode ? '✅ **Code generated by Amazon Bedrock (Claude 3 Haiku)**' : '⚠️ **Code generation failed - manual implementation required**'}
