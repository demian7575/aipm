...
export function createApp() {
  ...
  app.post('/api/merge-requests', (req, res, next) => {
    try {
      ...
      const result = store.createMergeRequest(payload);
  ...
  app.put('/api/merge-requests/:id', (req, res, next) => {
    try {
      ...
      const result = store.updateMergeRequest(req.params.id, payload);
      res.json(result);
    } catch (error) {
      next(error);
    }
  ...
  app.patch('/api/merge-requests/:id/status', (req, res, next) => {
    try {
      ...
      const result = store.updateMergeRequestStatus(req.params.id, status);
      res.json(result);
    } catch (error) {
      next(error);
    }
  ...
  app.post('/api/merge-requests/:id/update-branch', (req, res, next) => {
    try {
      const result = store.toggleDrift(req.params.id);
      res.json(result);
    } catch (error) {
      next(error);
    }
  ...
  app.post('/api/stories', (req, res, next) => {
    try {
      const base = UserStorySchema.omit({ depth: true, order: true, createdAt: true, updatedAt: true });
      const payload = base.parse(req.body);
      const result = store.createStory(payload);
      res.status(201).json(result);
    } catch (error) {
      next(error);
    }
  ...
  app.put('/api/stories/:id', (req, res, next) => {
    try {
      ...
      const result = store.updateStory(req.params.id, payload);
      res.json(result);
    } catch (error) {
      next(error);
    }
  ...
  app.patch('/api/stories/:id/move', (req, res, next) => {
    try {
      ...
      if (typeof index !== 'number') {
        throw badRequest('Index required');
      }
      const story = store.moveStory(req.params.id, parentId ?? null, index);
      res.json(story);
    } catch (error) {
      next(error);
    }
  ...
  app.patch('/api/stories/:id/reorder', (req, res, next) => {
    try {
      ...
      if (!Array.isArray(order)) {
        throw badRequest('Order must be array');
      }
  ...
  app.get('/api/stories/tree', (req, res, next) => {
    try {
      ...
      if (!mrId || typeof mrId !== 'string') {
        throw badRequest('mrId required');
      }
  ...
  app.post('/api/tests', (req, res, next) => {
    try {
      const base = AcceptanceTestSchema.omit({ createdAt: true, updatedAt: true });
      const payload = base.parse(req.body);
      res.status(201).json(store.createTest(payload));
    } catch (error) {
      next(error);
    }
  });
