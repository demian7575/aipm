...
2. **Functionality Separation**:
   ...
   - `openCreatePRModal()` - Focuses on PR creation with repository details, branch info, and PR title
   - Each modal has its own specific form fields and submission logic

3. **API Endpoints**:
   - Generate Code calls `/api/codewhisperer/generate-code`
   - Create PR calls `/api/codewhisperer/create-pr`
