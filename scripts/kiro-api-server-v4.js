...
function sendToKiroNoResponse(prompt) {
  if (!kiroProcess) {
    console.log('⚠️ Kiro CLI process not available');
    return;
  }
  
  console.log('📤 Sending prompt to Kiro CLI (no response expected)');
...
Work directly - no need to return JSON, just implement the feature and commit it.`;
            ...
            // Send to Kiro CLI without expecting JSON response
            sendToKiroNoResponse(kiroPrompt);
