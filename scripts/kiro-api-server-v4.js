...
After completing all operations, return ONLY this JSON format:
{"status": "Success", "message": "Code generated and pushed successfully"} 
OR
{"status": "Fail", "message": "Error description"}

Work directly and return the final status as JSON.`;
            ...
            // Call Kiro CLI and wait for status response
            try {
              const statusResult = await sendToKiroWithStatus(kiroPrompt);
              console.log('📥 Code generation status:', statusResult);
              
              // Update the response based on actual status
