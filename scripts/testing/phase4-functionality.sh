...
# ID: 1769786673633
# Acceptance Test: Button opens modal with all story titles
# =============================================================================
test_story_list_button_1769786673633() {
    ...
    # Test 5: Verify modal title is set to "Story List"
    if ! grep -q "title: 'Story List'" apps/frontend/public/app.js; then
        fail_test "Modal title not set to 'Story List'"
        return 1
    fi
    
    # Test 6: Verify clicking story navigates and closes modal
    if ! grep -q "selectStory(story.id)" apps/frontend/public/app.js; then
        fail_test "Story selection on click not implemented"
        return 1
    fi
    if ! grep -q "closeModal()" apps/frontend/public/app.js; then
        fail_test "Modal close on story click not implemented"
        return 1
    fi
...
if test_story_list_button_1769786673633; then
    ((PHASE4_PASSED++))
else
    ((PHASE4_FAILED++))
fi
