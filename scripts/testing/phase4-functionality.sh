...
if [ -n "$1" ]; then
  ...
  # Test for story 1770873614743: Story List Button
  if [ "$STORY_ID" == "1770873614743" ]; then
    ...
    echo "Test 45: Story list button event listener exists"
    if grep -q 'storyListBtn.addEventListener' apps/frontend/public/app.js; then
      echo "  ✅ PASS: Story list button event listener found"
      PASSED=$((PASSED + 1))
    else
