...
if [ -n "$1" ]; then
  ...
  # Test 46: Story selection on click
  echo "Test 46: Story click-to-select functionality"
  if grep -q 'selectStory(story.id)' apps/frontend/public/app.js; then
    echo "  ✅ PASS: Click-to-select functionality implemented"
    PASSED=$((PASSED + 1))
  else
    echo "  ❌ FAIL: Click-to-select functionality not found"
    FAILED=$((FAILED + 1))
  fi
fi
...
if [ -n "$1" ]; then
  echo "  - Story-Specific: 4 functionality tests"
fi
echo ""
if [ -n "$1" ]; then
