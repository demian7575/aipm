...
if [ -n "$1" ]; then
  ...
  # Test for Story 1772214671796: Add Story List Button
  if [ "$STORY_ID" = "1772214671796" ]; then
    ...
    if grep -q 'id="view-stories-btn"' apps/frontend/public/index.html; then
      p4_pass "story-1772214671796-1" "View Stories button exists in header" 0
    else
      p4_fail "story-1772214671796-1" "View Stories button missing from header" 0
    fi
    
    # Test 2: openStoryListModal function exists
    if grep -q "function openStoryListModal()" apps/frontend/public/app.js; then
      p4_pass "story-1772214671796-2" "openStoryListModal function implemented" 0
    else
      p4_fail "story-1772214671796-2" "openStoryListModal function missing" 0
    fi
    
    # Test 3: Pagination logic exists (20 items per page)
    if grep -q "ITEMS_PER_PAGE = 20" apps/frontend/public/app.js; then
      p4_pass "story-1772214671796-3" "Pagination with 20 items per page configured" 0
    else
      p4_fail "story-1772214671796-3" "Pagination configuration missing or incorrect" 0
    fi
    ...
    if grep -q "view-stories-btn.*addEventListener.*openStoryListModal" apps/frontend/public/app.js; then
      p4_pass "story-1772214671796-4" "View Stories button event listener attached" 0
    else
      p4_fail "story-1772214671796-4" "View Stories button event listener missing" 0
    fi
    
    # Test 5: CSS styling for story list modal
    if grep -q "\.story-list-modal" apps/frontend/public/styles.css; then
      p4_pass "story-1772214671796-5" "Story list modal CSS styling exists" 0
    else
      p4_fail "story-1772214671796-5" "Story list modal CSS styling missing" 0
    fi
