...
# Run story list view tests
if test_story_list_view; then
    ((PHASE4_PASSED++))
else
    ((PHASE4_FAILED++))
fi

if test_story_list_pagination; then
    ((PHASE4_PASSED++))
else
    ((PHASE4_FAILED++))
fi
...
if [ -n "$1" ]; then
    ...
    # Test: View Stories button exists in frontend
    echo "Test: View Stories button in HTML"
    if grep -q 'id="view-stories-btn"' apps/frontend/public/index.html; then
        echo "  ✅ PASS: View Stories button exists in HTML"
        ((PHASE4_PASSED++))
    else
        echo "  ❌ FAIL: View Stories button not found in HTML"
        ((PHASE4_FAILED++))
    fi
    
    # Test: View Stories button handler in JS
    echo "Test: View Stories button handler in JS"
    if grep -q 'viewStoriesBtn' apps/frontend/public/app.js; then
        echo "  ✅ PASS: View Stories button handler exists"
        ((PHASE4_PASSED++))
    else
        echo "  ❌ FAIL: View Stories button handler not found"
        ((PHASE4_FAILED++))
    fi
fi
