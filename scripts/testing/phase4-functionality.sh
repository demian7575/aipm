...
test_updated_user_authentication_with_oauth2() {
    ...
    # Test 2: Verify priority sorting endpoint
    ...
    if ! grep -q "priority" apps/frontend/public/app.js; then
        fail_test "Priority not displayed in frontend"
        return 1
    fi
    
    # Test 4: Verify sort button exists
    if ! grep -q "sort-priority-btn" apps/frontend/public/index.html; then
        fail_test "Sort by priority button not added"
        return 1
    fi
