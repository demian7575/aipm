...
# ============================================
# Story 1772167033486: Story List Button
# ============================================
if [ -z "$STORY_ID" ] || [ "$STORY_ID" = "1772167033486" ]; then
  echo "Testing Story 1772167033486: Story List Button"
  ...
  if echo "$FRONTEND_HTML" | grep -q 'id="story-list-btn"'; then
    p4_pass "story-1772167033486-button" "Story list button added to header" "$DURATION"
  else
    p4_fail "story-1772167033486-button" "Story list button missing from header" "$DURATION"
  fi
  
  # Check modal function exists
  if echo "$FRONTEND_JS" | grep -q "function openStoryListModal"; then
    p4_pass "story-1772167033486-modal" "Story list modal function implemented" "$DURATION"
  else
    p4_fail "story-1772167033486-modal" "Story list modal function missing" "$DURATION"
  fi
  
  # Check button event listener
  if echo "$FRONTEND_JS" | grep -q "storyListBtn.addEventListener"; then
    p4_pass "story-1772167033486-listener" "Button click handler implemented" "$DURATION"
  else
    p4_fail "story-1772167033486-listener" "Button click handler missing" "$DURATION"
  fi
  echo ""
fi
