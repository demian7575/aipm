...
phase2_step1_story_draft_generation() {
    ...
    local parent_story=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' "$API_BASE/api/stories" | jq -r '.[0] | select(.id != null) | .id')
    if [[ -z "$parent_story" ]]; then
        echo "   📝 No parent story found, creating one..."
        local create_result=$(curl -s -X POST "$API_BASE/api/stories" \
            -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' \
    ...
    local response
    response=$(timeout 330 curl -s -N -X POST "$SEMANTIC_API_BASE/aipm/story-draft?stream=true" \
        -H 'Content-Type: application/json' \
        -d "{
...
phase2_step2_create_story() {
    ...
    local response
    response=$(timeout 60 curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' -X POST "$API_BASE/api/stories" \
        -H 'Content-Type: application/json' \
        -d "$story_payload")
...
EOF
    ...
    local response
    response=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' -X PUT "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID" \
        -H 'Content-Type: application/json' \
        -d "$updated_payload")
...
phase2_step4_invest_analysis() {
    ...
    # Get story data
    local story_data=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID")
    ...
    local response
    response=$(timeout 330 curl -s -N -X POST "$SEMANTIC_API_BASE/aipm/invest-analysis?stream=true" \
        -H 'Content-Type: application/json' \
        -d "$payload" 2>&1)
...
phase2_step5_acceptance_test_draft() {
    ...
    local response
    response=$(timeout 330 curl -s -N -X POST "$SEMANTIC_API_BASE/aipm/acceptance-test-draft?stream=true" \
        -H 'Content-Type: application/json' \
        -d "$payload" 2>&1)
    ...
    local story_with_tests=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID")
...
EOF
    ...
    local response
    response=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' -X POST "$API_BASE/api/create-pr" \
        -H 'Content-Type: application/json' \
        -d "$pr_payload")
    ...
    local story_with_prs=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID")
...
phase2_step8_test_in_dev() {
    ...
    local story_check=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID")
...
phase2_step9_stop_tracking() {
    ...
    local response
    response=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' -X DELETE "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID/prs/$PHASE2_PR_NUMBER")
    
    # Verify PR was deleted
    sleep 2
    local verify=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID/prs")
...
phase2_step10_delete_story() {
    ...
    # Delete story
    local response
    response=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' -X DELETE "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID")
    
    # Verify deletion
    sleep 1
    local verify=$(curl -s -H 'X-Use-Dev-Tables: true' -H 'X-Project-Id: aipm' "$API_BASE/api/stories/$PHASE2_CHILD_STORY_ID")
